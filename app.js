...
const MOUNT_IDLE_TIMEOUT_MS = 2000;

const _mountQueue = [];

let _mountFront = false;

let _mountBack = false;
...
function nextMountIndex(background) {
  let best = -1;
  let bestRank = 99;
...
function drainMounts() {
  ...
  for (;;) {
    ...
    const idx = nextMountIndex(false);
    if (idx < 0) break;
    runMount(idx);
  ...
  scheduleBackgroundMounts(0);
}

function scheduleBackgroundMounts(delay) {
  if (_mountBack || !_mountQueue.length) return;
  _mountBack = true;
  const go = () => whenIdle(drainBackgroundMounts, MOUNT_IDLE_TIMEOUT_MS);
  if (delay > 0) setTimeout(go, delay);
  else go();
}

function drainBackgroundMounts() {
  _mountBack = false;
  if (!_mountQueue.length) return;
  if (nextMountIndex(false) >= 0) {
    kickMounts();
    return;
  }
  const wait = _prebuildHoldUntil - performance.now();
  if (wait > 0 || inputPending()) {
    scheduleBackgroundMounts(Math.min(Math.max(wait, 0) + 20, PREBUILD_RECHECK_MS));
    return;
  }
  const idx = nextMountIndex(true);
  if (idx < 0) {
    pruneMounts();
    scheduleBackgroundMounts(MOUNT_IDLE_MS);
    return;
  }
  runMount(idx);
  pruneMounts();
...
function warmTabPayloads() {
  ...
  if (state.activeTab !== OVERVIEW_TAB) {
    const overview = TABS.find((t) => t.id === OVERVIEW_TAB);
    if (overview) queue.push(overview);
  }
